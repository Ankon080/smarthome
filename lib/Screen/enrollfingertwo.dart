...
class EnrollFingerTwo extends StatefulWidget {
  const EnrollFingerTwo({super.key});
  @override
  State<EnrollFingerTwo> createState() => _EnrollFingerTwoState();
}

class _EnrollFingerTwoState extends State<EnrollFingerTwo> {
  ...
  void _checkEnrollmentStatus() {
    _dbRef.child('enrollment_status').onValue.listen((event) {
      final enrollmentStatus = event.snapshot.value;
      setState(() {
        if (enrollmentStatus == 1) {
          ...
          statusMessage = "Place the Same Finger Again";
          lottieAnimation = 'animations/scanfinger.json';
        } else if (enrollmentStatus == 3) {
          statusMessage = "Finger Templated";
          lottieAnimation = 'animations/scanfinger.json';
  ...
  Future<void> _updateRunStatus() async {
    await _dbRef.child('Enroll_fingerprint').set(6);
  }
            ...
            Padding(
              ...
              child: Row(
                children: [
                  GestureDetector(
                    onTap: () async {
                      await _updateRunStatus();

                      Navigator.pushAndRemoveUntil(
                        context,
                        MaterialPageRoute(
                          builder: (context) => const enrollfinger(),
                        ),
                            (route) => false,
                      ); // Navigate back
                    },
